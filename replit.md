...
## Twilio SMS Verification (OTP Authentication)

The application uses Twilio Verify API for phone number verification. **OTP verification is required for both login and registration** to ensure users own the phone numbers they provide.

### Authentication Flow (Secure 2-Step Process)
1. **Login Flow**:
   - User enters phone number and password
   - System validates credentials and sends OTP via SMS
   - User enters 6-digit OTP code to complete login
   - Session is created only after OTP verification
   - Security: Challenge is bound to session and validated credentials, invalidated on any failure

2. **Registration Flow**:
   - User enters name, phone number, and password
   - System validates data and sends OTP via SMS
   - User enters 6-digit OTP code to complete registration
   - Account is created only after OTP verification
   - Security: All registration data is bound to challenge, payload manipulation is prevented

### Security Features
- **Challenge Binding**: OTP challenges are bound to session and validated credentials
- **Single Attempt OTP**: Invalid OTP invalidates the challenge, requiring a new code request
- **10-Minute Expiry**: Challenges expire after 10 minutes
- **Brute-Force Prevention**: Any credential validation failure invalidates the challenge
- **Password Protection**: Password hashes are never exposed in API responses
...
**Login (2-step process):**
- `POST /api/auth/login/request-otp` - Validate credentials and send OTP
  - Body: `{ "phoneNumber": "+855123456789", "password": "..." }`
  - Response: `{ "message": "OTP sent successfully", "phoneNumber": "...", "requiresOtp": true }`

- `POST /api/auth/login` - Verify OTP and complete login
  - Body: `{ "phoneNumber": "+855123456789", "password": "...", "otpCode": "123456" }`
  - Response: User object (without password)

**Registration (2-step process):**
- `POST /api/auth/register/request-otp` - Validate data and send OTP
  - Body: `{ "fullName": "...", "phoneNumber": "+855123456789", "password": "..." }`
  - Response: `{ "message": "OTP sent successfully", "phoneNumber": "...", "requiresOtp": true }`

- `POST /api/auth/register` - Verify OTP and create account
  - Body: `{ "fullName": "...", "phoneNumber": "+855123456789", "password": "...", "otpCode": "123456" }`
  - Response: User object (without password)
