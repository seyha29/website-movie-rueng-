...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  function normalizePhoneNumber(phone: string): string {
    const digitsOnly = phone.replace(/^(\+855|855|0)/, '');
  ...
  app.post("/api/auth/request-otp", async (req, res) => {
    try {
      ...
      if (!['login', 'register'].includes(purpose)) {
        return res.status(400).json({ error: "Invalid purpose. Must be 'login' or 'register'" });
      }
      ...
      if (purpose === 'register') {
        const existingUser = await storage.getUserByPhoneNumber(normalizedPhone);
        if (existingUser) {
      ...
      const expiresAt = Math.floor(Date.now() / 1000) + 300; // 5 minutes expiry
      
      // Store OTP in database
      await storage.createOtpCode(normalizedPhone, otpCode, purpose, expiresAt);
      
      // Clean up expired OTP codes
      await storage.deleteExpiredOtpCodes();
      
      console.log(`[OTP] Code generated for ${normalizedPhone}: ${otpCode} (purpose: ${purpose})`);
      
      // Since we don't have SMS service yet, return the code for testing
      // In production, you would send this via SMS and NOT return it
      res.json({ 
        success: true, 
        message: "OTP code sent successfully",
        phoneNumber: normalizedPhone,
        // Remove this in production when you have SMS service:
        testCode: otpCode,
        expiresIn: 300
      });
    } catch (error) {
  ...
  app.post("/api/auth/verify-otp", async (req, res) => {
    try {
      ...
      // Get stored OTP
      const storedOtp = await storage.getOtpCode(normalizedPhone, purpose);
      ...
      if (storedOtp.code !== code) {
        ...
        const remainingAttempts = 4 - storedOtp.attempts;
        return res.status(400).json({ 
          error: `Invalid OTP code. ${Math.max(0, remainingAttempts)} attempts remaining.` 
        });
      }
      
      // Mark as verified (on success, don't increment attempts)
      await storage.markOtpVerified(storedOtp.id);
